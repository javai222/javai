...
// SOLID: DIP — Mocks stand in for EmployeeRepository / EmployeeFactory ports, not concrete DB or builders.
class EmployeeServiceImplTest {
    ...
    @ParameterizedTest
    @CsvSource({"404", "999"})
    void shouldThrowWhenMissingRecordForIds(long id) {
        // Arrange
    ...
    @Test
    void shouldThrowWhenUpdatingMissingEmployee() {
        //Arrange
        when(employeeRepository.findById(404L)).thenReturn(Optional.empty());

        //Assert
        assertThrows(ResourceNotFoundException.class, () -> employeeService.updateEmployee(404L, requestDTO));
